...
export async function GET() {
  try {
    ...
    const requestsWithProgress = requests.map(request => {
      ...
      const initialAmount = Number(request.amountWanted) || 0
      ...
      const supportersAmount = (request.requestSupports?.length || 0) * (Number(request.amountWanted) || 0)
      ...
      return {
        ...request,
        totalRequested,
        progressPercentage: request.goal ? Math.min(100, (totalRequested / Number(request.goal)) * 100) : 0,
        remainingNeeded: request.goal ? Math.max(0, Number(request.goal) - totalRequested) : 0
      }
    })
